...
fn main {
  ...
  // Initialize
  client.initialize(transport, fn(resp) { println("Initialized!") })
  ...
  client.open_document(
    transport, "file:///path/to/file.mbt", "moonbit", 1, "fn main { }",
  )
...
///|
fn main {
  let config = @server.ServerConfig::{ name: "my-lsp-server", version: "0.1.0" }
...
test "client with stub transport" {
  let config = @client.ClientConfig::{ root_uri: "file:///test", process_id: 1 }
  let client = @client.Client::new(config)
...
test "server with stub io" {
  let config = @server.ServerConfig::{ name: "test", version: "0.1.0" }
  let server = @server.Server::new(config)
